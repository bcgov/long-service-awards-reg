...
export default function AwardOptionsInput({
  award,
  confirm,
  cancel,
  regControl,
  ...
  const confirmOptions = (selectedOptions) => {
    ...
    const engravingSize =
      selectedOptions.hasOwnProperty("options") && selectedOptions.options;
    ...
    const filteredOptions = (options || [])
      .filter(({ type, name, value, customizable }) => {
        ...
        return (
          (selectedOptions.hasOwnProperty(type) && customizable) ||
  ...
  const getCurrentValue = (field, key) => {
    ...
    const { custom_value, award_option } =
      ((award && award.id) === (currentAward && currentAward.id) &&
        selections.find(({ award_option }) => {
          return (
            award_option.hasOwnProperty(key) && field === award_option[key]
          );
        })) ||
      {};
            ...
            <Controller
              ...
              render={({ field, fieldState: { invalid, error } }) => (
                <>
                  ...
                  />
                  <div>
                    <small>Maximum {option.value} characters.</small>
                  </div>
        ...
        {Object.keys(awardOptions || {}).map((key, index) => {
          ...
          return (
            <div
              ...
              className={"col-12 form-field-container"}
            >
    ...
    <div className={"grid"}>
      <form onSubmit={handleSubmit(confirmOptions)}>
        ...
        {Object.keys(errors || {}).length > 0 && (
          <p className={"error"}>
            The form has errors. Please check the fields above.
          </p>
        )}
      </form>
    </div>
