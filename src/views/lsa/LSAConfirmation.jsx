  ...
  return <FormStep steps={steps} previous={previous} current={current}>
    {
      completed && <Panel className={'mb-3'} header={'Confirm Registration Details'}>
        <FormData form={'lsa'} id={'milestone'}><MilestoneData/></FormData>
        <FormData form={'lsa'} id={'profile'}><ProfileData/></FormData>
        <FormData form={'lsa'} id={'contact'}><ContactData/></FormData>
          {
            previous_registration
                ? <Panel className={'mb-3'} header={'Award'}>Award Previously Selected</Panel>
                : <FormData form={'lsa'} id={'awards'}><AwardData/></FormData>
          }
        <FormData form={'lsa'} id={'supervisor'}><SupervisorData/></FormData>
      </Panel>
    }
