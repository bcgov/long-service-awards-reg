...
export default function SelfServicePins() {
  ...
  const BlockUITemplateConfirmed = () => {
    return (
      <Button
        onClick={() => {
          navigate("/service-pins/self/confirmation");
        }}
        icon={"pi pi-lock"}
      ...
      <>
        ...
        <Button
          onClick={async () => {
            ...
            //registration.notes =
            //"Registrant has opted to not register for the LSA.";
            registration.service.service_pin = true;
    ...
    return (
      <>
        <Card title="LSA Confirmation" footer={footer}>
          Please confirm whether you want to register for your Long Service
          Award, or continue with your Service Pin registration.
        </Card>
      </>
    );
  };
  ...
  return (
    <>
      <PageHeader
        title="Service Pin Registration"
        subtitle={(step && step.description) || ""}
      />
      <BlockUI
        blocked={lsaEligible && !confirmed && !lsaEligibleIgnore}
        template={BlockUITemplateLSA}
      >
        <BlockUI
          blocked={confirmed && step && step.key !== "confirmation"}
          template={BlockUITemplateConfirmed}
        >
