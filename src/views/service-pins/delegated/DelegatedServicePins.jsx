...
export default function DelegatedServicePins() {
  ...
  return (
    <FormProvider {...methods}>
      <form>
        <PageHeader
          title="Supervisor Service Pin Registration"
          subtitle={schema && schema.description}
        />
          ...
          <Panel
            ...
            header={
              <span className={"text-pink-500"}>
                Your Previous Service Pin Registrations
              </span>
            }
          >
            {(registrations || []).map((item, index) => (
              <EmployeeData
                key={`employee-registration-${index}`}
                data={item}
              />
            ))}
          </Panel>
        ...
        >
          ...
          <Panel className="container mt-3 mb-3" header={"Submit Registration"}>
            <div>
              {!isValid && (
                <p className={"error"}>
                  Form is incomplete or your input is not valid. Please update
                  before submitting.
                </p>
              )}
              <div className={"grid"}>
